...
Twenty ul of serial dilutions of the three sample types were plated on 1/10 tryptic soy broth (TSB) agar plates amended with cycloheximide to inhibit fungal growth and DMSO (control for MBOA) to assess total community size. The plates were incubated for five days at room temperature and colonies were counted on both media. The counts were multiplied by the respective dilution factor and the factor of the volume plated 50 and normalized to the weight. The CFU counts on TSB represent the total bacterial counts in the extracts which are culturable on this media in these conditions.

To quantify the amount of bacteria which are able to metabolize MBOA to a red pigment, 50 ul of the root samples were plated at a dilution of 1:100 and the rhizosphere samples at a dilution of 1:1000 and incubated for 10 days at room temperature. All colonies were counted and red counts were separately recorded. 
...
```{r functions, include=FALSE, echo=F, message=F, warning=F}
give.n <- function(x){
...
```{r data formatting, warning=F, echo=F, include=FALSE, message=F}
...
# data %<>% mutate(CFU_red_10d_norm_per = (CFU_red_10d / CFU_TSB_Chex_6d_w)* 100)
```

Percent mean calculations
```{r, echo=FALSE, include=FALSE, message=FALSE}
# since the Arabidopsis roots were too light to weigh with the balance, the weight was corrected to the same weight for all roots to approximate, normalized AMPO CFUs were calculated from these values

data %<>% mutate(sample_weight = case_when(species %in% "Ara" ~ 0.5,
                                          !species %in% "Ara" ~ sample_weight)) %>% 
         mutate(CFU_TSB_Chex_6d_w_n = CFU_TSB_Chex_6d/sample_weight) %>% 
         mutate(CFU_red_10d_norm_per = (CFU_red_10d / CFU_TSB_Chex_6d_w_n)* 100)
```
...
data %>% 
  filter(compartment %in% "root") %>% 
  filter(!sample_ID %in% c("B73_WT-root-2", "B73_WT-root-8", "Tri-root-1", "Bra-root-1")) %>% 
  ggplot(aes(x = species, y = log10(CFU_TSB_Chex_6d_w_n))) +
 geom_bar(aes(fill = species), alpha = 0.5, stat = "summary", position = "dodge", show.legend = FALSE) +
  geom_quasirandom(aes(fill = species), alpha = 0.7, shape = 21,  width = 0.3, show.legend = FALSE)+
  geom_errorbar(stat = "summary", position = position_dodge(width = 1), width=0.2) +
      stat_compare_means(method = "anova", aes(x = species, y = log10(CFU_TSB_Chex_6d_w_n)), label.y = 17, cex = 8/.pt, label.x = 1) +
...
ggsave(plot = AMPO_root, filename = "Fig1b_root.svg", width = 2.9, height = 6, dpi = 300, scale = 1, units = "cm")
...
ggsave(plot = AMPO_rhizo, filename = "Fig1b_rhizosphere.svg", width = 2.9, height = 6, dpi = 300, scale = 1, units = "cm")
...
ggsave(plot = AMPO_soil, filename = "Fig1b_soil.svg", width = 2.9, height = 6, dpi = 300, scale = 1, units = "cm")
...
CFU_AMPO_species_nobx1.aov <- data %>%  
  ...
  aov(CFU_red_10d_norm_per ~ species, data = .)
...
ggsave(plot = AMPO_species, filename = "AMPO_species.svg", width = 8.8, height = 6, dpi = 300, scale = 1, units = "cm")
```
